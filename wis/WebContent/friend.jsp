		...
		<div id="dd">
			...
			<table border="1px" width="100%" id="membertb">
				<tr align="center" bgColor="#ffffff" id="tbhead">
					<td width="15%" height="35">사람id</td>
					<td width="15%" height="35">친구id</td>
					<td width="15%" height="35">전화번호소유</td>
				</tr>
				<c:choose>
					<c:when test="${searchFriend == null}">
						<c:forEach var="fi" items="${friendmap}">
							<c:forEach var="friend" items="${fi.value}">
								<tr>
									<td>${fi.key}</td>
									<td>${friend.friendId}</td>
									<td>${friend.hasPhoneNum}</td>
							</c:forEach>
						</c:forEach>
					</c:when>
					<c:otherwise>
