		...
		<div id="dd">
			...
			<table border="1px" width="100%" id="membertb">
				<tr align="center" bgColor="#ffffff" id="tbhead">
					<td width="10%" height="35">방no</td>
					<td width="10%" height="35">댓글no</td>
					<td width="10%" height="35">글쓴이id</td>
					<td width="15%" height="35">댓글내용</td>
				</tr>
				<c:choose>
					<c:when test="${searchComment == null}">
						<c:forEach var="ci" items="${commentmap}">
							<c:forEach var="comment" items="${ci.value}">
								<tr>
									<td>${ci.key}</td>
									<td>${comment.no}</td>
